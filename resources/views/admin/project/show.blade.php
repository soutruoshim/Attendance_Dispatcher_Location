    ...
    <div class="float-end">
        ...
        @can('edit_project')
            <a href="{{route('admin.projects.edit',$projectDetail->id)}}" >
                <button class="btn btn-sm btn-success" ><i class="link-icon" data-feather="edit"></i> Edit project</button>
            </a>
        @endcan

        @can('create_task')
            <a href="{{route('admin.project-task.create',$projectDetail->id)}}" >
                <button class="btn btn-sm btn-secondary" ><i class="link-icon" data-feather="plus"></i> Create Task</button>
            </a>
        @endcan

        @can('upload_project_attachment')
            <a href="{{route('admin.project-attachment.create',$projectDetail->id)}}" >
                <button class="btn btn-sm btn-primary" ><i class="link-icon" data-feather="clipboard"></i> Upload Attachments</button>
            </a>
        @endcan
    </div>
@endsection
                                ...
                                <div class="col-lg-2 col-md-2 ps-0 d-flex">
                                    @forelse($value->assignedMembers as $key => $memberDetail)
                                        <img class="rounded-circle checklist-image" style="object-fit: cover"
                                             src="{{ $memberDetail->user->avatar ? asset(\App\Models\User::AVATAR_UPLOAD_PATH.$memberDetail->user->avatar)
