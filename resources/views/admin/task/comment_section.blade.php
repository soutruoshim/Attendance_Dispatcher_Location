<div class="row align-items-center justify-content-between">
    ...
    <div class="col-lg-7">
        ...
        <p class="showComments ">
            <mark class="text-muted p-1 px-2 rounded"><span class="commentsCount">{{count($comments)}}</span> Comments</mark>
        </p>
    </div>
    <div class="col-lg-3">
        <button class="btn btn-default btn-secondary float-end" id="createComment">Comment</button>
    </div>
</div>

<div class="commentListing list d-none mt-4">
    <div class="row align-items-center mb-4 border-bottom pb-3 commentsAdd">
        @forelse($comments as $key => $comment)
            <div class="comment-box parentComment{{$comment->id}}">
                ...
                <div class="comment-content rounded w-100">
                        ...
                        <div class="row number-reply d-flex align-items-center justify-content-between ">
                            <div class="col-lg-6">
                                <p class="text-muted pt-1" id="showReply" data-id="{{$comment->id}}">
                                    <span class="replyCount{{$comment->id}}">{{count($comment->replies) ?? 0}} </span> reply
                                </p>
                            </div>
                    ...
                    @can('delete_comment')
                        <a class="commentDelete" id="deleteComment" data-title="Comment" data-comment="{{$comment->id}}" data-id="{{$comment->id}}" href="{{route('admin.comment.delete',$comment->id)}}">
                            <i class="link-icon" data-feather="x"></i>
                        </a>
                    @endcan
                </div>
        ...
        <form id="taskCommentForm" action="{{route('admin.task-comment.store')}}" method="post" enctype="multipart/form-data" >
            ...
            <div class="comment-section">
                <div class="row align-items-center rounded border">
                    <div class="col-lg-3 select-comment">
                    ...
                    <div class="col-lg-7 ps-0">
                        <div class="position-relative">
                            <textarea class="form-control border-0 rounded-0 pe-5 ps-0" id="description" name="description" rows="2" cols="50" required
